...
import { batchBookmarks, exportLibrary, exportMigrationPackage, getActionMode, importLibrary, importMigrationPackage, initialize, initialized, listBookmarks, listCollections, listConflicts, resolveConflict, restoreBookmark, saveBookmark, saveBookmarkWithCollection, saveCollection, setSyncSettings, syncSettings, trashBookmark, webdavSettings, type ActionMode, type Bookmark, type BookmarkBatchAction, type BookmarkConflictChoices, type Collection } from "../local/db";
...
function App({ kind }: { kind: Surface }) {
  ...
  const exportMigration = async () => { try { setError(""); download(await exportMigrationPackage(), "private-bookmarks-migration.json"); } catch (reason) { setError(reason instanceof Error ? reason.message : "迁移包导出失败"); } };
  const importMigration = async (file: File) => { if (!window.confirm("导入迁移包会替换当前本地资料库。继续吗？")) return; try { setError(""); await importMigrationPackage(await file.text()); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "迁移包导入失败"); } };
